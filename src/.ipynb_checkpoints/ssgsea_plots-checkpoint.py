...
def score_per_cluster(adata, ssgsea_df, gene_set,  # column name in ssgsea_df
    library_id: str = '', res: str = 'hires', colour: str = 'multires_clst', agg: str = 'mean',  # or 'median'
    cmap: str = 'bwr', title: str = '', save_path: str = '.'):
    ...
    sc = ax.scatter(coords[:, 0],  coords[:, 1], c=spot_scores, cmap=cmap, s=2, edgecolor='none')

    ax.set_title(title if title else f"{gene_set} ({agg} NES per cluster)")
    ax.axis("off")
...
def ssgsea_scores(adata, ssgsea_df, gene_set,  # column name in ssgsea_df
    library_id: str = '', res: str = 'hires', colour: str = 'multires_clst', cmap: str = 'bwr', title: str = '', save_path: str = '.'):
    ...
    # Optional: center at 0.5 if your data is 0–1
...
def gsea_clusters(adata, ssgsea_df, library_id: str = '', res: str='hires', colour: str = 'multires_clst', k: int = 20, alpha: float=0.0, lw: float=0.9,
    ...
    fig, ax = plt.subplots(1, 1, figsize=(6, 6), dpi=200)
    ax.imshow(image)
    sq.pl.spatial_scatter(adata, color=colour, img_res_key='hires', size=1, cmap=custom_cmap, alpha=1, ax=ax, dpi=200)
    ax.set_title("Tissue Overlay")
    ax.axis("off")
    ...
    for adata_file in adata_files:
        ...
        for col in adata.obs.columns:
            if 'clusters_' in col:
                ...
                score_per_cluster(adata, ssgsea_df, gene_set, colour=col, save_path=output_path / f"ssgsea_cluster_score_{base_name}_k={k}.png")  # plot the mean score per cluster
                gsea_clusters(adata, ssgsea_df, colour=col, k=k, title=f'σ={params['sigma']}, {params['modality']}', save_path=output_path / f"clusters_{base_name}_k={k}.png")
